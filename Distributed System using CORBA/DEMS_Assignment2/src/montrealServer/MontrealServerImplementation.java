...
public class MontrealServerImplementation extends IDLInterfacePOA {
...
//			Finding capacity for particular event and increasing by one
	...
	//			cancel old event//		    book new event
				...
				reply1 = send("swapevent", newEventType, newEventId, torontoPort, customerId, 0, oldEventType, oldEventId);
				message = message + "\n" + reply1;
	...
	LOG.info("Montreal server sending request  "+key+" with Event Type: "+eventType+" Event Id: "+eventId+" Server Port: "+serverPort+" User Id: "+userID+" Capacity: "+capacity+ " Old Event Type: " + oldEventType + " Old Event Id: " + oldEventId);
...
//		Check if new event is valid and its capacity not zero
		if (newEventPrefix.equalsIgnoreCase("MTL")) {
		 ...
		 } else if (newEventPrefix.equalsIgnoreCase("TOR")) {
				reply1 = send("checkifvalid", newEventType, newEventId, torontoPort, customerId, 0,  "null", "null");
...
//		Check if event limit not reached
	...
	private synchronized boolean checkOldEventBooking(String customerId, String oldEventType, String oldEventId) {
		...
		String oldEventPrefix=oldEventId.substring(0, 3);
