...
public class OttawaServerImplementation  implements ServerInterface {
	...
	public Logger LOG = Logger.getLogger(OttawaServerImplementation.class.getName());
	...
	public void createLoggingFile(String system) {
		...
		file = new File("C:\\Users\\\\m_vadari\\Desktop\\Distributed system\\Server\\logs");
	...
	public OttawaServerImplementation() {
		...
		createLoggingFile("Ottawa");

//		eventInfo.put("CONFERENCES", new LinkedHashMap<String, Integer>());
//		eventInfo.get("CONFERENCES").put("OTWA100519", dummyCapacity);
//		eventInfo.get("CONFERENCES").put("OTWE100519", dummyCapacity);
//
//		eventInfo.put("SEMINARS", new LinkedHashMap<String, Integer>());
//		eventInfo.get("SEMINARS").put("OTWA100519", dummyCapacity);
//		eventInfo.get("SEMINARS").put("OTWE100519", dummyCapacity);
//
//		eventInfo.put("TRADE SHOWS", new LinkedHashMap<String, Integer>());
//		eventInfo.get("TRADE SHOWS").put("OTWA100519", dummyCapacity);
//		eventInfo.get("TRADE SHOWS").put("OTWE100519", dummyCapacity);
//
//		userInfo.put("OTWC2345", new LinkedHashMap<String, ArrayList<String>>());
//		userInfo.get("OTWC2345").put("CONFERENCES", new ArrayList<String>() {
//			{
//				add("OTWA100519");
//			}
//		});
//		userInfo.get("OTWC2345").put("SEMINARS", new ArrayList<String>() {
//			{
//				add("OTWA100519");
//			}
//		});
	...
	public synchronized String addEvent(String eventId, String eventType, int bookingCapacity) {
		...
		if ((eventId.subSequence(0, 3).toString()).equalsIgnoreCase("OTW")) {
			...
			if (eventInfo.containsKey(eventType)) {
				...
				if (eventInfo.get(eventType).containsKey(eventId)) {
					...
					message = "success| Event with Event ID:  " + eventId + "  has been successfully updated in the DEMS";
					LOG.info("SUCCESS");

				} else {
					eventInfo.get(eventType).put(eventId, bookingCapacity);
					message = "success| New event with event type: " + eventType + " and event id as " + eventId
				...
				eventInfo.get(eventType).put(eventId, bookingCapacity);
				message = "success| Event with Event ID:  " + eventType + "  has been successfully added in the system";
				LOG.info("SUCCESS");
			}
			LOG.info(message);

		} else if (eventPrefix.equalsIgnoreCase("TOR")) {
			reply1 = send("addeventmanager", eventType, eventId, torontoPort, null, bookingCapacity, null, null);
			message = reply1;
			...
			message = "failure| Event can not be added into the system.";
	...
	public synchronized String removeEvent(String eventType, String eventId) {
		...
		if (eventPrefix.equalsIgnoreCase("OTW")) {
...
//					Removing from other servers userInfo map
					reply1 = send("removeuserevent", eventType, eventId, torontoPort, null, 0, null, null);
					reply2 = send("removeuserevent", eventType, eventId, montrealPort, null, 0, null, null);
					message = reply1 + "\n" + reply2;
					message = "success| Successfully removed EventID. All associated users have been dealt with.";
					deletionFlag = true;
				}
			}
			if (!deletionFlag) {
				message = "failure| event id: " + eventId + " does not exist in the system so cannot perform deletion";
			}

		} else if (eventPrefix.equalsIgnoreCase("TOR")) {

			reply1 = send("deleteevent", eventType, eventId, torontoPort, null, 0, null, null);
			message = reply1;
			...
			message = "failure| Event removal failed as no match found.";
...
//		Printing other servers event availability using UDP 
			reply = send("listeventmanager", eventType, null, torontoPort, null, 0, null, null);
			message = message + reply + "\n";
			reply = send("listeventmanager", eventType, null, montrealPort, null, 0, null, null);
			message = message + reply + "\n";
			...
			if (userInfo.containsKey(customerId)) {
				if (userInfo.get(customerId).containsKey(eventType)) {
					if (userInfo.get(customerId).get(eventType).contains(eventId)) {
						...
						message = "failure| User already has been regstered for the event. User can not book same event twice.";
...
//					Decremented capacity by one
							...
							message = "success |Event ID: " + eventId
									+ " has been successfully issued to the user with user ID: " + customerId + ";\n";
							LOG.info("----SUCCESS----");
							LOG.info(message);

//							return message;
						} else {
							message = "failure| Event is full";
							LOG.info("Event is full");
//					return message;
						}
					} else {
						message = "failure| Event does not exist in the server";
...
//				return message;
			...
			if (checkUserBookingLimit(customerId, eventId)) {
				reply1 = send("bookevent", eventType, eventId, torontoPort, customerId, 0, null, null);
				message = reply1;
				...
				if (message.length() < 5) {
					message = "failure| Customer can book at most 3 events from other cities overall in a month.";
				}
			}
			...
			message = "failure| book event operation failed as no match found. Please try again.";
	...
	private boolean checkUserBookingLimit(String customerId, String eventId) {
		...
		for (String currEvent : currentUserEvents) {
			if (currEvent.substring(6, 8).equalsIgnoreCase(checkDate)
					&& !(currEvent.substring(0, 3).equalsIgnoreCase("OTW"))) {
				limitCounter++;
			}
		}
		if (limitCounter >= 3) {
...
//			Finding capacity for perticular event and increasing by one
					...
					if (eventTypeEntry.getKey().equalsIgnoreCase(eventType)) {
						for (Map.Entry<String, Integer> eventEntry : eventTypeEntry.getValue().entrySet()) {
							if (eventEntry.getKey().equalsIgnoreCase(eventId)) {
								...
								message = "success| Event has been canceled and capacity increased by 1. \n";
							}
						}
					}
				}
				...
				reply1 = send("cancelevent", eventType, eventId, torontoPort, customerId, 0, null, null);
				message = reply1;
			...
			message = "failure| Event does not exist for provided customerId";
		}
	...
	public synchronized String swapEvent(String customerId, String newEventId, String newEventType, String oldEventId, String oldEventType) {
		...
		if (checkOldEventBooking(customerId, oldEventType, oldEventId)) {
			foundFlag = true;
		}

		if (foundFlag) {
//			    book new event//			cancel old event
			...
			if (message.contains("success")) {
				...
				LOG.info("Event swapped successfully");
			}
		} else {
			LOG.info("Event swapping can't be done.");
			message = "failure| Event swapping can't be done.";
		}
		LOG.info("The Result is: ");
		LOG.info(message);
		LOG.info("Sending reply back to the client (User) ");
	...
	String send(String key1, String eventType, String eventId, int serverPort, String userID, int capacity,
			String oldEventType, String oldEventId) {
		LOG.info("Ottawa server sending request  " + key1 + " with Event Type: " + eventType + " Event Id: " + eventId
				+ " Server Port: " + serverPort + " User Id: " + userID + " Capacity: " + capacity + " Old Event Type: "
				+ oldEventType + " Old Event Id: " + oldEventId);
		DatagramSocket aSocket = null;
		String messageReceived = null;
		String key = null;
		try {

			key = key1 + "," + eventType + "," + eventId + "," + serverPort + "," + userID + "," + capacity + ","
					+ oldEventType + "," + oldEventId;
		...
		LOG.info("Ottawa server got the respose back " + messageReceived);
...
//		Checking if customerId having booking for oldEventId and type
		...
		if (!result) {
			message = "customer don't have event booking for event Id " + oldEventId + "\n";
		}
