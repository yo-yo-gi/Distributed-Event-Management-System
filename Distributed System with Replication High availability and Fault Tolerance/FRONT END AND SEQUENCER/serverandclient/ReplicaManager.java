...
public class ReplicaManager {
	...
	private static ServerImpl monStub;
	private static ServerImpl ottStub;
	private static ServerImpl torStub;
	private static  Queue<String> requestQueue; 	
/* things to change */	private static final int FEPORT=4001, CRASH_SERVER_PORT=9001, MULTICAST_RECEIVE_PORT=8001; 
...
/* things to change */	public static void main(String[] args) {
		try {
			...
			//monStub=new ServerImpl("MTL", 6660);
			//ottStub=new ServerImpl("OTW", 8880);
			//torStub=new ServerImpl("TOR", 7770);
	...
	private static String executeRequest(String inputData){
		...
		ServerImpl targetStub = null;
		String initialID = executorId.substring(0, 3).toUpperCase();
		...
		case "C_SWAP_EVENT":
			...
			break;
		case "M_ADD_EVENT":
			result += targetStub.addEvent(eventId, eventType, String.valueOf(bookingCapacity));
			break;
		case "M_REMOVE_EVENT":
...
//			throw new Exception("No operation matching");
	...
	private static void stopServers() throws Exception{
		...
		monStub.UDPServerInstance.stopServer();
		ottStub.UDPServerInstance.stopServer();
		torStub.UDPServerInstance.stopServer();
