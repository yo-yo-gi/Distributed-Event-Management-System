...
public class MontrealServer {
	...
	Registry registry = LocateRegistry.createRegistry(3636);
	registry.bind("montrealStub", monStub);

	System.out.println("Montreal City Server has been started successfully");
	...
	try {
		...
		monStub.LOG.info("Montreal server received request  "+operation+" with Event Type: "+eventType+" Event Id: "+eventId+" Server Port: "+port+" User Id: "+customerId+" Capacity: "+bookingCapacity);
...
//				For only listing availability of event from eventInfo only by manager
		    ...
		    monStub.LOG.info("---------Request recieved to lsit the availability of the event by manager---------");
		    message = "";
