...
public class MontrealServerImplementation extends UnicastRemoteObject implements RMIInterface {
	...
	userInfo.get("MTLC2345").put("TRADE SHOWS", new ArrayList<String>() {
	    {
		add("MTLA100519");
		add("TORA100519");
	    }
	...
	if ((eventId.subSequence(0, 3).toString()).equalsIgnoreCase("MTL")) {
	    ...
	    reply1 = send("addeventmanager", eventType, eventId, torontoPort, null, bookingCapacity);
...
//					Removing from other servers userInfo map
		    reply1 = send("removeuserevent", eventType, eventId, torontoPort, null, 0);
	    ...
	    reply1 = send("deleteevent", eventType, eventId, torontoPort, null, 0);
	    message = reply1;
...
//		Printing other servers event availability using UDP 
	reply = send("listeventmanager", eventType, null, torontoPort, null, 0);
	message = message + reply + "\n";
	reply = send("listeventmanager", eventType, null, ottawaPort, null, 0);
	message = message + reply + "\n";
...
//				return message;
		...
		reply1 = send("bookevent", eventType, eventId, torontoPort, customerId, 0);
		message = reply1;
...
//			Finding capacity for perticular event and increasing by one
		...
		reply1 = send("cancelevent", eventType, eventId, torontoPort, customerId, 0);
		message = message + "\n" + reply1;
	...
	LOG.info("Montreal server sending request  "+key+" with Event Type: "+eventType+" Event Id: "+eventId+" Server Port: "+serverPort+" User Id: "+userID+" Capacity: "+capacity);
