...
public class ClientImplementation {
    ...
    Registry torontoRegistry;
    Registry ottawaRegistry;
    Registry montrealRegistry;
    RMIInterface torStub, ottStub, montStub;
    String result;
    String initialID;
    String id;
	...
	try {
	    torontoRegistry = LocateRegistry.getRegistry(4343);
	    ottawaRegistry = LocateRegistry.getRegistry(3535);
	    montrealRegistry = LocateRegistry.getRegistry(3636);

	    torStub = (RMIInterface) torontoRegistry.lookup("torontoStub");
	    ottStub = (RMIInterface) ottawaRegistry.lookup("ottawaStub");
	    montStub = (RMIInterface) montrealRegistry.lookup("montrealStub");
//	    testMulti();	//just to test multithreading at time of  demo
	...
	if (M.matches()) {
	    ...
	    System.out.println(
		    "Enter Valid ID. Item ID must consist of initial three letters of (TOR/OTW/MTL) any city followed by unique four digits(0-9).");
    ...
    /**
     * To test muti threading in assignment at time of demo
     */
    public void testMulti() {
    	Thread t1=new Thread(new Runnable() {
			...
			public void run() {
				try {
					montStub.bookEvent("MTLC1234", "CONFERENCES","MTLA100519");
...
Thread t2=new Thread(new Runnable() {
			...
			public void run() {
				try {
					montStub.bookEvent("MTLC1235", "CONFERENCES","MTLA100519");
				} catch (RemoteException e) {
					e.printStackTrace();
				}
...
Thread t3=new Thread(new Runnable() {
	...
	public void run() {
		try {
			montStub.bookEvent("MTLC1236", "CONFERENCES","MTLA100519");
		} catch (RemoteException e) {
			e.printStackTrace();
		}
...
Thread t4=new Thread(new Runnable() {
	...
	public void run() {
		try {
			montStub.bookEvent("MTLC1237", "CONFERENCES","MTLA100519");
		} catch (RemoteException e) {
			e.printStackTrace();
		}
...
Thread t5=new Thread(new Runnable() {
	...
	public void run() {
		try {
			montStub.bookEvent("MTLC1238",  "CONFERENCES","MTLA100519");
		} catch (RemoteException e) {
			e.printStackTrace();
		}
