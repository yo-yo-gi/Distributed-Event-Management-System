...
public class OttawaServerImplementation extends UnicastRemoteObject implements RMIInterface {
	...
	userInfo.get("OTWC2345").put("TRADE SHOWS", new ArrayList<String>() {
	    {
		add("OTWE100519");
	    }
	});
...
//					Removing from other servers userInfo map
		    ...
		    reply2 = send("removeuserevent", eventType, eventId, montrealPort, null, 0);
		    message = reply1 + "\n" + reply2;
...
//		Printing other servers event availability using UDP 
	for (String typeOfEvent : eventInfo.keySet()) {
	    reply = send("listeventmanager", typeOfEvent, null, torontoPort, null, 0);
...
//			Finding capacity for perticular event and increasing by one
	...
	LOG.info("Ottawa server sending request  "+key+" with Event Type: "+eventType+" Event Id: "+eventId+" Server Port: "+serverPort+" User Id: "+userID+" Capacity: "+capacity);
